...
theme.whenComponentLoaded = (component, callback) => {
  ...
  components.forEach((c) =>
    observer.observe(c, { attributes: true, attributeFilter: ["loaded"] })
  );
};
...
const CartForm = class extends HTMLElement {
  ...
  refreshFromHtml(html) {
    ...
    this.dispatchEvent(
      new CustomEvent("on:cart:after-merge", {
        bubbles: true,
        cancelable: false,
      })
    );
...
const FilterContainer = class extends HTMLElement {
  constructor() {
    ...
    if (utilityBar) {
      ...
      this.throttledCheckStickyScroll = theme.throttleEvent(
    ...
    if (this.filters) {
      ...
      this.allowAutoApplyHideUnavailable =
        this.filters.dataset.autoApplyHideUnavailable;
    ...
    this.filters.querySelectorAll("input, select").forEach((input) => {
      if (
        ((input.type !== "checkbox" && input.type !== "radio") ||
          input.checked) && // is an active input value
  ...
  ajaxLoadUrl(url, noPushState) {
    ...
    fetch(fetchUrl, {
      ...
      .then((response) => {
        ...
        document.dispatchEvent(new CustomEvent("on:filter:after-ajax-load"));
      });
  }
};
...
const LinkDropdown = class extends HTMLElement {
  ...
  addDismissListener() {
    this.dismissCallback = this.toggle.bind(this);
    ...
    theme.addDelegateEventListener(
      ...
      (evt, delEl) => {
        ...
        this.mobileDrawer
          ...
          .scrollTo({ top: 0, left: 0, behavior: "instant" }); // 'smooth' not working in iOS 15
      }
    );
...
const ProductBlock = class extends HTMLElement {
  ...
  initCustomLazyLoading() {
    ...
    this.addEventListener("touchstart", this.boundLazyLoadAll, {
      passive: true,
    });
  }
...
const MediaGallery = class extends HTMLElement {
  ...
  openGalleryViewer(evt, imageLink) {
    ...
    this.galleryModal
      .querySelector("gallery-viewer")
      .init(imageLink.getAttribute("href"));
  }
  ...
  setActiveMediaGroup(groupName) {
    ...
    this.mediaLists.forEach((list) => {
      list.forEach((mediaItem) => {
        const mediaItemGroup = mediaGroupData.media[mediaItem.dataset.mediaId];
        const showThis =
          mediaItemGroup === groupName || mediaItemGroup === true;
        mediaItem.hidden = !showThis;
      });
      if (list.length > 0) {
...
if (theme.settings.quickbuyStyle !== "off") {
  theme.quickbuy = {
          ...
          if (window.innerWidth >= 768) {
            ...
            if (block.classList.contains("expanded")) {
              quickbuyCont.dispatchEvent(
                new CustomEvent("on:quickbuy:before-open", { bubbles: true })
              );
              ...
              fetch(productUrl, {
                ...
                .then((response) => {
                  ...
                  quickbuyCont.dispatchEvent(
                    new CustomEvent("on:quickbuy:after-open", { bubbles: true })
                  );
                });
    ...
    contractDetail: (block, instant) => {
      ...
      quickbuyCont.dispatchEvent(
        new CustomEvent("on:quickbuy:after-close", { bubbles: true })
      );
    },
  ...
  constructor() {
    ...
    if (this.fullHeightContainer) {
      ...
      if (phOffsetTop - 5 > thisOffsetTop) {
        // Must be lower. For trans header, it'll roughly match
        this.fullHeightContainer.classList.add(
          "height--full-ignore-header-height"
        );
        return;
